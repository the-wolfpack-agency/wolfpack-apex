/**
 * /api/automations/[automationId]/summaries/[classKey]/export-pdf — handler tests.
 *
 * Mirrors the .docx sibling's pattern: mock requireCapability, registry,
 ...
 * `src/lib/automations/porsche-classes/__tests__/export-pdf.test.ts`,
...
jest.mock("@/lib/automations/porsche-classes/export-pdf", () => ({
  renderClassSummaryPdf: (...args: unknown[]) => mockRender(...args),
}));

import { NextRequest, NextResponse } from "next/server";
import { GET } from "@/app/api/automations/[automationId]/summaries/[classKey]/export-pdf/route";

function req(): NextRequest {
  return new NextRequest(
    "http://test/api/automations/porsche-classes/summaries/BA101%7C2026-04-13%7CWestlake/export-pdf",
    { method: "GET" },
  );
}
...
describe("GET /api/automations/[automationId]/summaries/[classKey]/export-pdf", () => {
  ...
  it("500 when the renderer throws", async () => {
    ...
    mockRender.mockRejectedValueOnce(new Error("pdf boom"));
    const errorSpy = jest.spyOn(console, "error").mockImplementation(() => {});
    const res = await GET(req(), {
      params: Promise.resolve({
        automationId: "porsche-classes",
  ...
  it("200 returns pdf bytes + Content-Type + Content-Disposition", async () => {
    mockRequireCapability.mockResolvedValueOnce({
      ok: true,
      user: USER,
      capabilities: new Set(),
    ...
    mockAssemble.mockResolvedValueOnce(FAKE_SUMMARY);
    // Real-ish PDF-magic prefix so a downstream sniff (curl/file) is happy.
    ...
    expect(res.headers.get("Content-Disposition")).toBe(
      'attachment; filename="class-summary-BA101_2026-04-13_Westlake.pdf"',
    );
