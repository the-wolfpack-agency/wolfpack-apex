...
jest.mock("@/lib/auth/require-capability", () => ({ requireSession: (...a: unknown[]) => requireSession(...a) }));
...
beforeEach(() => { [requireSession, getCourseBySlug, setLessonProgress, getCourseProgress, trackEvent].forEach((m) => m.mockReset()); });

describe("POST /api/lms/progress", () => {
  it("401 when unauthenticated", async () => {
    requireSession.mockResolvedValueOnce({ ok: false, response: new Response("", { status: 401 }) });
  ...
  it("records a completion and returns fresh progress", async () => {
    requireSession.mockResolvedValueOnce(OK);
  ...
  it("rejects an invalid action with 400", async () => {
    requireSession.mockResolvedValue(OK);
