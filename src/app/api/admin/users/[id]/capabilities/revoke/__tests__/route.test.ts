/**
 * Contract tests for POST /api/admin/users/[id]/capabilities/revoke.
 *
 * Locked behaviors:
 *   - 403 without admin.roles.assign
 *   - 400 invalid capability
 *   - 404 unknown user (persist fails with DATABASE_URL set)
 *   - 200 happy path: revoke applied, analytics + AUDIT recorded with the right
 *     action + afterState (security-relevant capability revoke → hash chain)
 */
...
jest.mock("@/lib/auth/capability-overrides", () => ({
  ...
  applyRevoke: (...a: any[]) => mockApplyRevoke(...a),
...
const EXISTING = { grants: ["settings.manage_team"], revokes: [] };
const NEXT = { grants: [], revokes: ["settings.manage_team"] };

beforeEach(() => {
  jest.clearAllMocks();
  mockRecordAudit.mockResolvedValue(undefined);
  mockLoad.mockResolvedValue({ overrides: EXISTING });
  mockApplyRevoke.mockReturnValue(NEXT);
...
describe("POST /api/admin/users/[id]/capabilities/revoke", () => {
  it("403 without admin.roles.assign", async () => {
    mockRequireCap.mockResolvedValue({
      ok: false,
      response: new Response(JSON.stringify({ error: "forbidden" }), { status: 403 }),
    });
    const { POST } = await import("@/app/api/admin/users/[id]/capabilities/revoke/route");
