...
jest.mock("@/lib/platform-scan/manifests", () => ({ listScanTargets: () => mockList() }));
...
beforeEach(() => {
  ...
  mockList.mockReturnValue([{ platform: "wolfpack-auto", baseUrl: "https://x", hasStatic: true }]);
});

it("returns the scan targets for an authorized admin", async () => {
