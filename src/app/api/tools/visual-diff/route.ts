...
export async function POST(req: NextRequest) {
  ...
  const paths = (body.paths ?? ["/", "/login", "/sites", "/security-posture"]).join(",");
  ...
  try {
    ...
    if ("error" in result) {
      trackEvent("tools.visual_diff_run", user.id, user.role, { success: false, error: result.error });
      return NextResponse.json({ status: "failed", error: result.error }, { status: 500 });
    }

    trackEvent("tools.visual_diff_run", user.id, user.role, { success: true, run_id: result.run_id });
    return NextResponse.json({ status: "queued", run_id: result.run_id });
  } catch (err) {
    const msg = (err as Error).message ?? "";
    trackEvent("tools.visual_diff_run", user.id, user.role, { success: false, error: msg.slice(0, 200) });
    return NextResponse.json(
