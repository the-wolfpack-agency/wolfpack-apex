-- Tier 4 DB rename: apex_team_members → instinct_team_members
--
...
BEGIN
  ...
  SELECT c.relkind INTO instinct_kind
    ...
    WHERE c.relname = 'instinct_team_members' AND n.nspname = current_schema() LIMIT 1;

  IF apex_exists_as_table THEN
    EXECUTE 'SELECT COUNT(*) FROM apex_team_members' INTO pre_rename_count;
  ELSIF instinct_kind = 'r' THEN
    EXECUTE 'SELECT COUNT(*) FROM instinct_team_members' INTO pre_rename_count;
  END IF;
  ...
  IF instinct_kind = 'r' AND NOT apex_exists_as_table THEN
    EXECUTE 'CREATE OR REPLACE VIEW apex_team_members AS SELECT * FROM instinct_team_members';
    RAISE NOTICE 'Already renamed; ensured compat view apex_team_members exists.';
  ELSIF apex_exists_as_table AND instinct_kind = 'v' THEN
    EXECUTE 'DROP VIEW instinct_team_members';
    EXECUTE 'ALTER TABLE apex_team_members RENAME TO instinct_team_members';
    EXECUTE 'CREATE OR REPLACE VIEW apex_team_members AS SELECT * FROM instinct_team_members';
    RAISE NOTICE 'Renamed apex_team_members → instinct_team_members; compat view created.';
  ELSIF apex_exists_as_table AND instinct_kind IS NULL THEN
    EXECUTE 'ALTER TABLE apex_team_members RENAME TO instinct_team_members';
    EXECUTE 'CREATE OR REPLACE VIEW apex_team_members AS SELECT * FROM instinct_team_members';
    RAISE NOTICE 'Renamed apex_team_members → instinct_team_members (no pre-existing alias); compat view created.';
  ELSIF instinct_kind IS NOT NULL AND instinct_kind NOT IN ('r', 'v') THEN
    RAISE EXCEPTION 'Refusing to proceed: instinct_team_members exists as relkind=% — manual inspection required.', instinct_kind;
  ELSIF NOT apex_exists_as_table AND instinct_kind = 'v' THEN
    EXECUTE 'DROP VIEW instinct_team_members';
    RAISE NOTICE 'Dropped orphan view instinct_team_members; apex_team_members absent.';
  ELSE
    RAISE NOTICE 'Neither apex_team_members nor instinct_team_members exists; nothing to rename.';
  END IF;

  IF pre_rename_count IS NOT NULL THEN
    BEGIN
      EXECUTE 'SELECT COUNT(*) FROM instinct_team_members' INTO post_rename_count;
...
BEGIN
  FOR r IN
    ...
    WHERE schemaname = current_schema() AND indexname LIKE 'idx_apex_team_members_%'
  LOOP
    EXECUTE format('ALTER INDEX %I RENAME TO %I', r.indexname,
                    replace(r.indexname, 'idx_apex_team_members_', 'idx_instinct_team_members_'));
  END LOOP;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = current_schema() AND table_name = 'apex_team_members'
  ) THEN
    SELECT v.is_updatable, v.is_insertable_into INTO is_updatable, is_insertable
      FROM information_schema.views v
      WHERE v.table_schema = current_schema() AND v.table_name = 'apex_team_members';
    IF is_updatable != 'YES' THEN
      RAISE EXCEPTION 'Compat view apex_team_members not updatable (is_updatable=%)', is_updatable;
    END IF;
    IF is_insertable != 'YES' THEN
      RAISE EXCEPTION 'Compat view apex_team_members not insertable (is_insertable_into=%)', is_insertable;
    END IF;
    RAISE NOTICE 'Compat view apex_team_members fully R/W.';
  END IF;
END $$;
