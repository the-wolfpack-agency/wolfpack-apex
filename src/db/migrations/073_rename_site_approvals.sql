-- Tier 4 DB rename: apex_site_approvals → instinct_site_approvals
--
...
BEGIN
  SELECT EXISTS (
    ...
    WHERE c.relname = 'apex_site_approvals' AND c.relkind = 'r' AND n.nspname = current_schema()
  ) INTO apex_exists_as_table;

  SELECT c.relkind INTO instinct_kind
    FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE c.relname = 'instinct_site_approvals' AND n.nspname = current_schema() LIMIT 1;

  IF apex_exists_as_table THEN
    EXECUTE 'SELECT COUNT(*) FROM apex_site_approvals' INTO pre_rename_count;
  ELSIF instinct_kind = 'r' THEN
    EXECUTE 'SELECT COUNT(*) FROM instinct_site_approvals' INTO pre_rename_count;
  END IF;
  ...
  IF instinct_kind = 'r' AND NOT apex_exists_as_table THEN
    EXECUTE 'CREATE OR REPLACE VIEW apex_site_approvals AS SELECT * FROM instinct_site_approvals';
    RAISE NOTICE 'Already renamed; ensured compat view apex_site_approvals exists.';
  ELSIF apex_exists_as_table AND instinct_kind = 'v' THEN
    EXECUTE 'DROP VIEW instinct_site_approvals';
    EXECUTE 'ALTER TABLE apex_site_approvals RENAME TO instinct_site_approvals';
    EXECUTE 'CREATE OR REPLACE VIEW apex_site_approvals AS SELECT * FROM instinct_site_approvals';
    RAISE NOTICE 'Renamed apex_site_approvals → instinct_site_approvals; compat view created.';
  ELSIF apex_exists_as_table AND instinct_kind IS NULL THEN
    EXECUTE 'ALTER TABLE apex_site_approvals RENAME TO instinct_site_approvals';
    EXECUTE 'CREATE OR REPLACE VIEW apex_site_approvals AS SELECT * FROM instinct_site_approvals';
    RAISE NOTICE 'Renamed apex_site_approvals → instinct_site_approvals (no pre-existing alias); compat view created.';
  ELSIF instinct_kind IS NOT NULL AND instinct_kind NOT IN ('r', 'v') THEN
    RAISE EXCEPTION 'Refusing to proceed: instinct_site_approvals exists as relkind=% — manual inspection required.', instinct_kind;
  ELSIF NOT apex_exists_as_table AND instinct_kind = 'v' THEN
    EXECUTE 'DROP VIEW instinct_site_approvals';
    RAISE NOTICE 'Dropped orphan view instinct_site_approvals; apex_site_approvals absent.';
  ELSE
    RAISE NOTICE 'Neither apex_site_approvals nor instinct_site_approvals exists; nothing to rename.';
  END IF;

  IF pre_rename_count IS NOT NULL THEN
    BEGIN
      EXECUTE 'SELECT COUNT(*) FROM instinct_site_approvals' INTO post_rename_count;
...
BEGIN
  FOR r IN
    ...
    WHERE schemaname = current_schema() AND indexname LIKE 'idx_apex_site_approvals_%'
  LOOP
    EXECUTE format('ALTER INDEX %I RENAME TO %I', r.indexname,
                    replace(r.indexname, 'idx_apex_site_approvals_', 'idx_instinct_site_approvals_'));
  END LOOP;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = current_schema() AND table_name = 'apex_site_approvals'
  ) THEN
    SELECT v.is_updatable, v.is_insertable_into INTO is_updatable, is_insertable
      FROM information_schema.views v
      WHERE v.table_schema = current_schema() AND v.table_name = 'apex_site_approvals';
    IF is_updatable != 'YES' THEN
      RAISE EXCEPTION 'Compat view apex_site_approvals not updatable (is_updatable=%)', is_updatable;
    END IF;
    IF is_insertable != 'YES' THEN
      RAISE EXCEPTION 'Compat view apex_site_approvals not insertable (is_insertable_into=%)', is_insertable;
    END IF;
    RAISE NOTICE 'Compat view apex_site_approvals fully R/W.';
  END IF;
END $$;
