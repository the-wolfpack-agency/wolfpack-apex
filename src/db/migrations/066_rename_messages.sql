-- Tier 4 DB rename: apex_messages → instinct_messages
--
...
BEGIN
  ...
  SELECT c.relkind INTO instinct_kind
    ...
    WHERE c.relname = 'instinct_messages' AND n.nspname = current_schema() LIMIT 1;

  IF apex_exists_as_table THEN
    EXECUTE 'SELECT COUNT(*) FROM apex_messages' INTO pre_rename_count;
  ELSIF instinct_kind = 'r' THEN
    EXECUTE 'SELECT COUNT(*) FROM instinct_messages' INTO pre_rename_count;
  END IF;
  ...
  IF instinct_kind = 'r' AND NOT apex_exists_as_table THEN
    EXECUTE 'CREATE OR REPLACE VIEW apex_messages AS SELECT * FROM instinct_messages';
    RAISE NOTICE 'Already renamed; ensured compat view apex_messages exists.';
  ELSIF apex_exists_as_table AND instinct_kind = 'v' THEN
    EXECUTE 'DROP VIEW instinct_messages';
    EXECUTE 'ALTER TABLE apex_messages RENAME TO instinct_messages';
    EXECUTE 'CREATE OR REPLACE VIEW apex_messages AS SELECT * FROM instinct_messages';
    RAISE NOTICE 'Renamed apex_messages → instinct_messages; compat view created.';
  ELSIF apex_exists_as_table AND instinct_kind IS NULL THEN
    EXECUTE 'ALTER TABLE apex_messages RENAME TO instinct_messages';
    EXECUTE 'CREATE OR REPLACE VIEW apex_messages AS SELECT * FROM instinct_messages';
    RAISE NOTICE 'Renamed apex_messages → instinct_messages (no pre-existing alias); compat view created.';
  ELSIF instinct_kind IS NOT NULL AND instinct_kind NOT IN ('r', 'v') THEN
    RAISE EXCEPTION 'Refusing to proceed: instinct_messages exists as relkind=% — manual inspection required.', instinct_kind;
  ELSIF NOT apex_exists_as_table AND instinct_kind = 'v' THEN
    EXECUTE 'DROP VIEW instinct_messages';
    RAISE NOTICE 'Dropped orphan view instinct_messages; apex_messages absent.';
  ELSE
    RAISE NOTICE 'Neither apex_messages nor instinct_messages exists; nothing to rename.';
  END IF;

  IF pre_rename_count IS NOT NULL THEN
    BEGIN
      EXECUTE 'SELECT COUNT(*) FROM instinct_messages' INTO post_rename_count;
...
BEGIN
  FOR r IN
    ...
    WHERE schemaname = current_schema() AND indexname LIKE 'idx_apex_messages_%'
  LOOP
    EXECUTE format('ALTER INDEX %I RENAME TO %I', r.indexname,
                    replace(r.indexname, 'idx_apex_messages_', 'idx_instinct_messages_'));
  END LOOP;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = current_schema() AND table_name = 'apex_messages'
  ) THEN
    SELECT v.is_updatable, v.is_insertable_into INTO is_updatable, is_insertable
      FROM information_schema.views v
      WHERE v.table_schema = current_schema() AND v.table_name = 'apex_messages';
    IF is_updatable != 'YES' THEN
      RAISE EXCEPTION 'Compat view apex_messages not updatable (is_updatable=%)', is_updatable;
    END IF;
    IF is_insertable != 'YES' THEN
      RAISE EXCEPTION 'Compat view apex_messages not insertable (is_insertable_into=%)', is_insertable;
    END IF;
    RAISE NOTICE 'Compat view apex_messages fully R/W.';
  END IF;
END $$;
