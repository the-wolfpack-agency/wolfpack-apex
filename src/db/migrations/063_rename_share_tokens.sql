-- Tier 4 DB rename: apex_share_tokens → instinct_share_tokens
--
...
BEGIN
  ...
  SELECT c.relkind INTO instinct_kind
    ...
    WHERE c.relname = 'instinct_share_tokens' AND n.nspname = current_schema() LIMIT 1;

  IF apex_exists_as_table THEN
    EXECUTE 'SELECT COUNT(*) FROM apex_share_tokens' INTO pre_rename_count;
  ELSIF instinct_kind = 'r' THEN
    EXECUTE 'SELECT COUNT(*) FROM instinct_share_tokens' INTO pre_rename_count;
  END IF;
  ...
  IF instinct_kind = 'r' AND NOT apex_exists_as_table THEN
    EXECUTE 'CREATE OR REPLACE VIEW apex_share_tokens AS SELECT * FROM instinct_share_tokens';
    RAISE NOTICE 'Already renamed; ensured compat view apex_share_tokens exists.';
  ELSIF apex_exists_as_table AND instinct_kind = 'v' THEN
    EXECUTE 'DROP VIEW instinct_share_tokens';
    EXECUTE 'ALTER TABLE apex_share_tokens RENAME TO instinct_share_tokens';
    EXECUTE 'CREATE OR REPLACE VIEW apex_share_tokens AS SELECT * FROM instinct_share_tokens';
    RAISE NOTICE 'Renamed apex_share_tokens → instinct_share_tokens; compat view created.';
  ELSIF apex_exists_as_table AND instinct_kind IS NULL THEN
    EXECUTE 'ALTER TABLE apex_share_tokens RENAME TO instinct_share_tokens';
    EXECUTE 'CREATE OR REPLACE VIEW apex_share_tokens AS SELECT * FROM instinct_share_tokens';
    RAISE NOTICE 'Renamed apex_share_tokens → instinct_share_tokens (no pre-existing alias); compat view created.';
  ELSIF instinct_kind IS NOT NULL AND instinct_kind NOT IN ('r', 'v') THEN
    RAISE EXCEPTION 'Refusing to proceed: instinct_share_tokens exists as relkind=% — manual inspection required.', instinct_kind;
  ELSIF NOT apex_exists_as_table AND instinct_kind = 'v' THEN
    EXECUTE 'DROP VIEW instinct_share_tokens';
    RAISE NOTICE 'Dropped orphan view instinct_share_tokens; apex_share_tokens absent.';
  ELSE
    RAISE NOTICE 'Neither apex_share_tokens nor instinct_share_tokens exists; nothing to rename.';
  END IF;

  IF pre_rename_count IS NOT NULL THEN
    BEGIN
      EXECUTE 'SELECT COUNT(*) FROM instinct_share_tokens' INTO post_rename_count;
...
BEGIN
  FOR r IN
    ...
    WHERE schemaname = current_schema() AND indexname LIKE 'idx_apex_share_tokens_%'
  LOOP
    EXECUTE format('ALTER INDEX %I RENAME TO %I', r.indexname,
                    replace(r.indexname, 'idx_apex_share_tokens_', 'idx_instinct_share_tokens_'));
  END LOOP;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = current_schema() AND table_name = 'apex_share_tokens'
  ) THEN
    SELECT v.is_updatable, v.is_insertable_into INTO is_updatable, is_insertable
      FROM information_schema.views v
      WHERE v.table_schema = current_schema() AND v.table_name = 'apex_share_tokens';
    IF is_updatable != 'YES' THEN
      RAISE EXCEPTION 'Compat view apex_share_tokens not updatable (is_updatable=%)', is_updatable;
    END IF;
    IF is_insertable != 'YES' THEN
      RAISE EXCEPTION 'Compat view apex_share_tokens not insertable (is_insertable_into=%)', is_insertable;
    END IF;
    RAISE NOTICE 'Compat view apex_share_tokens fully R/W.';
  END IF;
END $$;
