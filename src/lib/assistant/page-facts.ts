...
export const PAGE_FACTS: Record<string, PageFact> = {
  ...
  invoices: {
    ...
    route: "/invoices",
    title: "Invoices",
    purpose: "One hub for every invoice surface: the Vendor Invoices AP queue (upload/scan/approve) and read-only SharePoint mirrors of company budget & SOW workbooks (PCNA today; more companies later, same hub).",
    what_you_can_do: [
      "Open Vendor Invoices to upload, review, approve, and mark AP invoices paid.",
      "Open a company tracker (e.g. PCNA) to see its Summary tab mirrored live from SharePoint, search it, and force a re-sync.",
      "Each sub-page is gated: the AP queue by finance access, each tracker by its viewer allowlist.",
    ],
    how_to: [
      "Open Invoices from the left nav; pick a sub-page card.",
      "Vendor Invoices for AP; a company name (PCNA) for the read-only SharePoint mirror.",
      "Use the breadcrumb (← Invoices) to return to the hub.",
    ],
