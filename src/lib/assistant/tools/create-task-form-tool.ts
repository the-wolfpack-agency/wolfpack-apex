 ...
 *
 * This tool always returns a FormSpec. For a human caller the chat surface
 * renders it. For an AGENT caller the GENERIC agent executor auto-fills the form
 * from the instruction and executes it on behalf of the owner via the secure
 * on-behalf token + the shared form executor (see
 * src/lib/agents/tasks/executor.ts). There is deliberately NO per-tool agent
 * branch here: every form-returning tool flows through the same generic path.
 */
...
export const createTaskFormTool: ToolDef<Params, CreateTaskFormData> = {
  ...
  async handler(params, ctx): Promise<ToolResult<CreateTaskFormData>> {
    /* Build the form against the EFFECTIVE user. For a normal human turn that
     * is the caller. For an agent dispatch the executor sets
     * onBehalfOfUserId = the owner, so the list dropdown is populated with the
     * OWNER's writable To-Do lists and the first one becomes the form's default
     * listId, which is what the generic executor then submits on the owner's
     * behalf. Falling back to the literal "default" string is avoided (Graph
     * rejects it with ErrorInvalidIdMalformed). */
    const effectiveUserId = ctx.onBehalfOfUserId ?? ctx.userId;

    let listOptions: TaskListOption[] = [];
    try {
      const lists = await listCachedTaskLists(effectiveUserId);
      listOptions = lists
    ...
    trackEvent("assistant.form_offered", ctx.userId, ctx.userRole, {
      ...
      on_behalf_of_owner: ctx.onBehalfOfUserId ? true : false,
    });
    return {
      ok: true,
