 ...
 *      changes `aria-current` on the folder button. We OBSERVE
 *      `/api/emails/inbox` requests during the click; if the inbox
 *      panel re-fetches, we assert the URL. If it does NOT (the
 *      current InboxPanel does not pass `folder` to the API), we
 *      record that as a finding in the test output. See the
 *      `folder click triggers re-fetch` test for details.
...
test.describe("emails inbox flow (real browser)", () => {
  ...
  test("folder click updates aria-current and (if wired) refetches the inbox", async ({
    ...
    // FINDING (documented in PR body): the current InboxPanel does
    // NOT pass a `folder` query param when the parent's activeFolder
    // changes — clicking Drafts/Sent/Archived only updates the
    // highlight, not the data. This assertion is INTENTIONALLY soft:
    // we record what we observed and surface it as an annotation.
    ...
    if (refetched) {
      // If the panel DOES refetch, validate the URL carries the folder.
